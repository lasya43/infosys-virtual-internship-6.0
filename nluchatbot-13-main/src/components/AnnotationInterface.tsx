      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-4">
            <Button onClick={handleAutoPredict} disabled={predicting} variant="secondary" className="w-full">
              {predicting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Sparkles className="h-4 w-4 mr-2" />}
              Auto-Predict Intent & Entities with AI
            </Button>

            {(intent || entities.length > 0) && (
              <div className="p-4 border rounded-md bg-muted/50">
                <h4 className="font-semibold mb-3 text-sm">AI Predictions:</h4>
                <div className="space-y-3">
                  {intent && (
                    <div>
                      <span className="text-xs text-muted-foreground">Predicted Intent:</span>
                      <Badge className="ml-2" variant="default">{intent}</Badge>
                    </div>
                  )}
                  {entities.length > 0 && (
                    <div>
                      <span className="text-xs text-muted-foreground">Predicted Entities ({entities.length}):</span>
